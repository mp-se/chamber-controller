...
char* fixedPointToString(char* s, long_temperature rawValue,
  ...
  int intPart = longTempDiffToInt(
      rawValue);  // rawValue is supposed to be without internal offset
  uint16_t fracPart;
  const char* fmt;
  uint16_t scale;
  switch (numDecimals) {
  ...
  fracPart = ((rawValue & TEMP_FIXED_POINT_MASK) * scale +
              TEMP_FIXED_POINT_SCALE / 2) >>
             TEMP_FIXED_POINT_BITS;  // add 256 for rounding
  if (fracPart >= scale) {
    intPart++;
    fracPart = 0;
  }
 ...
 *
 * \param numberString - String to convert (Either with units or in
 * tempControl.cc.tempFormat units) \returns converted temperature
 */
temperature stringToTemp(const char* numberString) {
  char tempBuffer[20] = "";
  char tempFormat = tempControl.getControlConstants().tempFormat;
...
long_temperature stringToFixedPoint(const char* numberString) {
  ...
  if (numberString[0] == '-') {
    numberString++;
    negative =
        true;  // by processing the sign here, we don't have to include strtol
  }

  // find the point in the string to split in the integer part and the fraction
  // part
...
/**
 * Receives the tempFormat temp format in fixed point and converts it to the
 * internal format It scales the value for Fahrenheit and adds the offset needed
 * for absolute temperatures. For temperature differences, use no offset.
 *
...
/**
 * Receives the external temp format in fixed point and converts it to the
 * internal format It scales the value for Fahrenheit and adds the offset needed
 * for absolute temperatures. For temperature differences, use no offset.
 *
 * @param rawTemp - Temperature to convert
...
long_temperature convertToInternalTempImpl(long_temperature rawTemp,
                                           bool addOffset) {
  return convertToInternalTempImpl(
      rawTemp, addOffset, tempControl.getControlConstants().tempFormat);
}

// convertAndConstrain adds an offset, then scales with *9/5 for Fahrenheit. Use
// it without the offset argument for temperature differences
long_temperature convertFromInternalTempImpl(long_temperature rawTemp,
                                             bool addOffset) {
  if (tempControl.getControlConstants().tempFormat ==
      'F') {  // value received is in F, convert to C
    if (addOffset) {
      rawTemp -= F_OFFSET;
    }
    rawTemp = rawTemp * 9 / 5;
...
/**
 * \brief Convert a fixed point temperature to decimal value rounded to nearest
 * tenth
 *
...
int fixedToTenths(long_temperature temp) {
  ...
  return static_cast<int>(
      (10 * temp + intToTempDiff(5) / 10) /
      intToTempDiff(1));  // return rounded result in tenth of degrees
}
 ...
 * @return The value of `valLong` if it is within the bounds.  `lower` if
 * `valLong` is below, `upper` if `valLong` is above.
 */
temperature constrainTemp(long_temperature valLong, temperature lower,
                          temperature upper) {
 ...
 * @return The value of `val` if it is within the bounds.  `MIN_TEMP` if `val`
 * is below, `MAX_TEMP` if `val` is above.
 * @see MIN_TEMP
 * @see MAX_TEMP
 */
...
temperature multiplyFactorTemperatureLong(temperature factor,
                                          long_temperature b) {
  return constrainTemp16(((long_temperature)factor * (b - C_OFFSET)) >>
                         TEMP_FIXED_POINT_BITS);
}

temperature multiplyFactorTemperatureDiffLong(temperature factor,
                                              long_temperature b) {
  return constrainTemp16(((long_temperature)factor * b) >>
                         TEMP_FIXED_POINT_BITS);
}
...
temperature multiplyFactorTemperatureDiff(temperature factor, temperature b) {
  return constrainTemp16(((long_temperature)factor * (long_temperature)b) >>
                         TEMP_FIXED_POINT_BITS);
}
...
double tempToDouble(long_temperature rawTemp, uint8_t numDecimals) {
  ...
  String temp(tempToString(tempString, rawTemp, numDecimals,
                           Config::TempFormat::bufferLen));
  temp.trim();
...
double tempDiffToDouble(long_temperature rawTempDiff, uint8_t numDecimals) {
  char tempString[Config::TempFormat::bufferLen];
  String temp(tempDiffToString(tempString, rawTempDiff, numDecimals,
                               Config::TempFormat::bufferLen));
  temp.trim();
  return temp.toDouble();
}
...
double fixedPointToDouble(long_temperature rawFixedPoint, uint8_t numDecimals) {
  char tempString[Config::TempFormat::bufferLen];
  String temp(fixedPointToString(tempString, rawFixedPoint, numDecimals,
                                 Config::TempFormat::bufferLen));
  temp.trim();
  return temp.toDouble();
}
